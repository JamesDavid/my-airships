...
export class Airship {
  ...
  buildMesh() {
    ...
    // ---- THE FRAMES THAT MAKE IT A GIRDER ----
    //
    // The keel was three parallel sticks with nothing between them. A
    // Santos-Dumont keel is a triangular-section girder of pine scantlings and
    // piano wire -- "the thin pine scantlings and piano wires of Nice had saved
    // my life" -- and what makes it a girder rather than three sticks is the
    // transverse frames that tie the longerons together, closed at each end.
    // Without them the three members read as unrelated, which is exactly what
    // a pilot asked about.
    //
    // Built off keelRails, so it follows whatever the keel actually is: three
    // members make a triangle, the Omnibus's five make her deeper section, and
    // the No. 9's keelson closes an inverted one under the basket. The end
    // frames are heavier, as they were -- they take the whole rake of the wires.
    if (this.keelRails.length >= 3) {
      const braceM = new THREE.MeshLambertMaterial({ color: 0xb9a273 });
      const struts = [];
      // the rails that run the length of her, in cross-section order round the
      // outside: sort by angle about their own centroid so the frame closes
      // without crossing itself whatever shape the section is
      const rails = this.keelRails.slice();
      const cy2 = rails.reduce((a, r2) => a + r2.y, 0) / rails.length;
      const cz2 = rails.reduce((a, r2) => a + r2.z, 0) / rails.length;
      rails.sort((a, b) => Math.atan2(a.y - cy2, a.z - cz2)
        - Math.atan2(b.y - cy2, b.z - cz2));
      const span = Math.min(...rails.map((r2) => r2.len));
      const N = Math.max(3, Math.round(span / 2.4));      // a frame every ~2.4 m
      // THE BAY WHERE THE CAR HANGS IS LEFT OPEN, which is both what a real
      // keel does and the only way this can be built at all: the check that
      // asks whether a keel member stands in the pilot caught the very first
      // version at -0.326 m on the No. 7 -- a strut from the apex down to a
      // lower rail runs through exactly the space his shoulders are in. A
      // girder with a car slung in it has no frame across that bay.
      const bxK = K.type === 'minimal' || K.type === 'basket-long' ? 0 : -1.2;
      for (let i = 0; i <= N; i++) {
        const x = -span / 2 + (i / N) * span;
        const end = i === 0 || i === N;
        if (!end && Math.abs(x - bxK) < 1.15) continue;
        const rad = end ? 0.045 : 0.028;
        for (let k = 0; k < rails.length; k++) {
          const a = rails[k], b = rails[(k + 1) % rails.length];
          if (rails.length === 2 && k === 1) break;       // two rails, one strut
          const dy = b.y - a.y, dz2 = b.z - a.z;
          const L2 = Math.hypot(dy, dz2);
          if (L2 < 0.05) continue;
          const m = new THREE.Mesh(new THREE.CylinderGeometry(rad, rad, L2, 4), braceM);
          m.position.set(x, (a.y + b.y) / 2, (a.z + b.z) / 2);
          // a cylinder stands along +Y; lay it along the chord of the section
          m.rotation.x = Math.atan2(dz2, dy) * -1 + Math.PI;
          m.rotation.x = -Math.atan2(dz2, dy);
          m.castShadow = true;
          this.pitchGroup.add(m);
          struts.push({ x, y0: a.y, z0: a.z, y1: b.y, z1: b.z, r: rad });
        }
      }
      this.keelFrames = N + 1;
      // published like the rails, and for the same reason: the check that asks
      // whether a keel member is standing in the pilot could only see the
      // LONGERONS, so a brand-new strut running from the apex down to a lower
      // rail could have gone straight through his shoulders unremarked. Every
      // new piece of structure has to be measurable or the check quietly stops
      // covering the thing it was written for.
      this.keelStruts = struts;
    }
